...
public class Trajectory1 : MonoBehaviour
{
    AutoMove autoMove;
    Character character;
    ...
    public float height = 1;       // 箭的最大高度 
    public Vector3 _startPos, _stopPos, _curPos;   // 起始位置，目標位置，當前位置
    ...
    void Update()
    {
        if(autoMove.enemy1!=null){
            endpoint = autoMove.enemy1.transform;
            _stopPos = endpoint.transform.position;
        }

        startpoint = transform.parent;        
        speed = character.now_AtkSpeed*2.5f;           //*3 保证画面真实
